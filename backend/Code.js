...
function putLargeCache(cacheKey, jsonStr) {
 ...
 try {
   if (jsonStr.length < 90000) {
     ...
     cache.put(cacheKey + "_count", chunks.length.toString(), 45);
     const dict = {};
     for (let j = 0; j < chunks.length; j++) {
       dict[cacheKey + "_" + j] = chunks[j];
     }
     cache.putAll(dict, 45);
   }
 } catch(e) {}
}
...
CacheService.getScriptCache().remove("recentOutings_" + ENV);
...
function getRecentOutingSheets() {
 const cacheKey = "recentOutings_" + ENV;
 const cache = CacheService.getScriptCache();
 ...
 // Cache Stampede Protection (Lock)
 const lock = LockService.getScriptLock();
 try {
   lock.waitLock(15000);
   ...
   cache.put(cacheKey, JSON.stringify(finalResult), 1800); // 30 minutes
   return finalResult;
...
function fetchManualPairingData(sheetUrl) {
 ...
 try {
   ...
   // 0. Extract Location Data from OutingInformation for Dropdowns
   let meetingLocs = [];
   let dismissalLocs = [];
   const infoSheet = ss.getSheetByName("OutingInformation");
   if (infoSheet) {
   ...
   // Global Extra Data Map for Long Press Modals & IC checks
   ...
   const result = { 
   success: true, 
   data: { trainees: trainees, volunteers: volunteers, meetingLocs: meetingLocs, dismissalLocs: dismissalLocs } 
   };
   putLargeCache(cacheKey, JSON.stringify(result));
   return result;
 } catch(e) {
...
function runSheetMaintenance(sheetUrl) {
 const lock = LockService.getScriptLock();
 try {
   if (!lock.tryLock(15000)) return; // If highly congested, gracefully skip sorting to prevent app lockups
   const ss = SpreadsheetApp.openByUrl(sheetUrl);
   let tSheet = ss.getSheetByName("Trainee Attendance");
   if (!tSheet) tSheet = ss.getSheetByName("Trainee Attendance ");
   const vSheet = ss.getSheetByName("Volunteer Attendance");
