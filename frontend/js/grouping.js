...
showOverlay('loading', 'Loading Groups...');
...
if (sortedGroups.length === 0) {
    container.innerHTML = '<p class="text-sm text-gray-500 italic text-center py-4">No groups assigned yet. Group some trainees first.</p>';
    return;
}
...
sortedGroups.forEach((g, index) => {
    const bgColorClass = 'bg-gray-50 dark:bg-zinc-800/50';
    
    // Find all trainees in this group
    let tList = activeTrainees.filter(t => String(t.group).trim() === g);
    
    // Find all volunteers logically attached to this group
    ...
    // Build options based on these volunteers
    let volOptionsHtml = '<option value="">-- No IC Assigned --</option>';
    let currentICName = "";
    
    Array.from(groupVolKeys).sort().forEach(vKey => {
        const vObj = volLookup.get(vKey);
        if (vObj) {
            const name = vObj.name;
            const remark = (vObj.extra && vObj.extra.remark) ? ` (Remarks: ${vObj.extra.remark})` : '';
            const isIC = vObj.groupIC === true;
            if (isIC) currentICName = name;
            volOptionsHtml += `<option value="${name.replace(/"/g, '&quot;')}" ${isIC ? 'selected' : ''}>${name}${remark}</option>`;
        }
    });

    html += `
    <div class="${bgColorClass} p-3 md:p-4 rounded-xl border border-gray-200 dark:border-zinc-700">
        <label class="block text-sm font-black text-gray-900 dark:text-white mb-2 uppercase tracking-wide">Group ${g} IC</label>
        <select onchange="handleAssignICChange('${g}', this.value)" class="w-full bg-white dark:bg-black border border-gray-300 dark:border-zinc-600 rounded-lg p-2.5 text-sm text-gray-900 dark:text-white shadow-sm focus:outline-none focus:border-blue-500">
            ${volOptionsHtml}
        </select>
    </div>
    `;
...
function handleAssignICChange(group, newVolName) {
lastGroupingLocalChange = Date.now();

// 1. Find all active trainees in this specific group
let tList = (groupingData.trainees || []).filter(t => t.attending === 'y' && !t.isGoneHome && String(t.group).trim() === String(group));

// 2. Identify all volunteers attached to these trainees
let groupVolKeys = new Set();
tList.forEach(t => {
    if (t.volPaired) {
        const vols = t.volPaired.split(/[,|\n]+/).map(v => v.trim()).filter(v => v);
...
// 3. For all volunteers in this group, reset their groupIC flag to false (unless it's the newly selected one)
(groupingData.volunteers || []).forEach(v => {
    const vKey = v.name.toLowerCase();
    
    // Only affect volunteers actually part of this group
    if (groupVolKeys.has(vKey)) {
        const shouldBeIC = (vKey === newVolKey);
        
        if (v.groupIC !== shouldBeIC) {
            v.groupIC = shouldBeIC;
            
            // Queue update
            const updateIndex = pendingGroupingUpdates.findIndex(u => u.name === v.name && u.role === 'VOLUNTEER');
            if (updateIndex > -1) {
                pendingGroupingUpdates[updateIndex].groupIC = shouldBeIC;
            } else {
                pendingGroupingUpdates.push({ role: 'VOLUNTEER', name: v.name, groupIC: shouldBeIC });
            }
        }
    }
});

// We can rely on closing the modal to trigger grouping sync natively, or trigger it directly. 
// Triggering directly gives better instant feedback for the comm user.
triggerGroupingSync();
}
