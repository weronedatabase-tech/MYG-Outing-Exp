...
// Shared across Pairing and Grouping Views
// ==========================================
...
if (!window.dndInitialized) {
    ...
    function startDrag(e, clientX, clientY, isTouch) {
        ...
        if(currentActiveView !== 'manual-pairing' && currentActiveView !== 'manual-grouping') return;
        ...
        if (dndState.isDragging && dndState.clone) {
            ...
            document.querySelectorAll('.dnd-dropzone').forEach(dz => {
                ...
                if (currentActiveView === 'manual-pairing' && dz.dataset.role !== undefined && dndState.el.dataset.role !== undefined && dz.dataset.role !== dndState.el.dataset.role) {
                    isValid = true;
                } else if (currentActiveView === 'manual-grouping' && dz.dataset.grouptarget !== undefined) {
                    isValid = true;
                }
        ...
        if (dndState.isDragging && dndState.clone) {
            ...
            if (dropZone && dndState.el) {
                if (currentActiveView === 'manual-pairing') {
                    ...
                    if (dropZone.dataset.grouptarget !== undefined) {
                        if (typeof handleGroupingDrop === 'function') {
                            handleGroupingDrop(dndState.el.dataset.name, dropZone.dataset.grouptarget);
                        }
                    }
                }
            }
