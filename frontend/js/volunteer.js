...
// Inline Optimistic Loading State
btn.disabled = true;
...
apiCall('submitAttendanceData', payload).then(res => { 
  if(res.success) {
      ...
      btn.classList.remove('bg-green-600', 'hover:bg-green-700', 'opacity-80', 'cursor-not-allowed');
      btn.classList.add('bg-blue-600', 'hover:bg-blue-700'); 
      
      showFlashMessage('volFormStatus', res.message, 'success');
      ...
      showFlashMessage('volFormStatus', res.message, 'error');
  }
}); 
}