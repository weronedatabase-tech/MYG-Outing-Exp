...
btn.innerHTML = `<i class="fa-solid fa-circle-notch fa-spin"></i> Saving in background...`;
btn.disabled = true;
...
setTimeout(() => {
  resetVolForm(); 
  document.getElementById('volNameSearch').value = ""; 
  showFlashMessage('volFormStatus', 'Attendance successfully updated! (Saving in background)', 'success');
}, 300);

apiCall('submitAttendanceData', payload).then(res => { 
  if(res.success) {
      if(selectedVolType === 'volunteer' && res.message.includes("added")) { 
          const url = document.getElementById('volSheetSelector').value; 
      ...
      alert("Background save failed for " + target + ": " + res.message);
  }
}); 
}